...
class Order extends CI_Model {
	...
	{
		return $this->db->query("SELECT orders.id, users.first_name, users.last_name, orders.created_at, street, city, state, zip FROM orders JOIN users ON orders.user_id=users.id JOIN billing ON users.billing_id=billing.id JOIN addresses ON billing_address_id=addresses.id JOIN order_products ON order_products.order_id = orders.id JOIN products ON products.id=order_products.product.id")->result_array();
	}
